...
image::RessourceAdoc/img/virementcompte.png[Alt code virement, 75%]
...
* La classe "EmployeManagement" est utilisée pour la création, la modification, la consultation et la suppression d'un employé. Elle est responsable de la manipulation et de la mise à jour des informations relatives à cet employé. Tout en affichant les fenêtres de création, modification, consultation et suppression.
...
```
En l'occurence ici, on utilise le clic droit pour afficher un menu contextuel avec les options "Modifier", "Supprimer" et "Consulter". On utilise également le double clic pour modifier un employé ou le consulter. Biensure les option proproser dépendent des droits de l'employé sélectionné sur l'employé sélectionné.
